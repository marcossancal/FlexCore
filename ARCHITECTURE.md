...
O FlexCore segue um padrão **MVC estruturado** sem framework externo. Toda a requisição HTTP entra por um único ponto (`index.php`), passa por guards de instalação e autenticação, e é despachada pelo Router para o Controller correto. O acesso ao banco de dados é centralizado em um wrapper PDO (`lib/DB.php`), e a lógica de negócio vive em Services com acesso a dados via Repositories.
...
index.php (entry point)
    ...
    ├─ Guard de autenticação   (Auth::check(), exceto /login)
    │
    ▼
...
view() → app/views/*.php → Response HTML
         (ou json_encode → Response JSON para a API)
```
...
```php
$container->bind(RecordService::class, function($c) {
...
- Métodos `GET`, `POST`, `any()`
- Parâmetros de rota `{slug}` extraídos por regex
- Middlewares por rota (interface `MiddlewareInterface`)
- Despacho para `[ClassName::class, 'method']` com resolução via Container
...
Router::dispatch()
  ...
  → instancia middlewares em cadeia
  → resolve o Controller via Container
  → chama o método com parâmetros de rota injetados
```
...
| `DB::run($sql, $params)` | `void` | UPDATE / DELETE |
...
| `RecordRepository` | Registros e seus valores |
...
O mesmo padrão se repete para update e delete.
...
| `EntityController` | CRUD de entidades e campos |
| `RecordController` | CRUD de registros de qualquer entidade |
| `SettingsController` | Configurações do sistema + CRUD de usuários |
| `ApiKeyController` | Gerenciamento de chaves de API |
...
**`ApiResponse`** — formata todas as respostas no padrão JSON:API:
```json
{
  "data":   { ... },
  "errors": null,
  "meta":   { "status": 200, "total": 42 }
}
```
...
O `boot()` é o ponto de entrada — é onde o plugin registra seus hooks, adiciona rotas ou configura serviços. O sistema não expõe mais nenhuma API obrigatória.
